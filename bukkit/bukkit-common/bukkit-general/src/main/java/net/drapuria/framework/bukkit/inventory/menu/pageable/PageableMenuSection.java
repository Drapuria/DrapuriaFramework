...
import net.drapuria.framework.bukkit.inventory.menu.buttons.PaginatedButton;
...
public abstract class PageableMenuSection<I> extends PageableGuiSection<I, IMenu, PaginatedButton> {
    private final int middle, next, last;
    private IButton defaultButton;

    protected PageableMenuSection(@NotNull IMenu menu, final int middle, final int next, final int last, @Nullable Button defaultButton) {
        super(menu);
        this.middle = middle;
        this.next = next;
        this.last = last;
    ...
    public void setDefaultButton(IButton defaultButton) {
        this.defaultButton = defaultButton;
    }

    public Map<Integer, IButton> buildPage() {
