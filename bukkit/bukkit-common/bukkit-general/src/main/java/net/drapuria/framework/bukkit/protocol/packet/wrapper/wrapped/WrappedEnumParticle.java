package net.drapuria.framework.bukkit.protocol.packet.wrapper.wrapped;
...
public enum WrappedEnumParticle {
    ...
    static {
        if (MinecraftVersion.getVersion().newerThan(Minecraft.Version.v1_9_R1)) {
            try {
            ...
            try {
                ENUM_PARTICLE_VALUE_OF = PARTICLE_CLASS_RESOLVER.resolveSilent(MinecraftVersion.getVersion().olderThan(Minecraft.Version.v1_13_R1) ? "EnumParticle" : "Particle").getDeclaredMethod("valueOf");
            } catch (NoSuchMethodException e) {
                e.printStackTrace();
            }
        }
